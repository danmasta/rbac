...
// Returns true if every element from iterator is truthy
...
// Returns true if every element from iterator is truthy, ignores null and undefined
function everyNotNil (arr, fn) {
    return every(arr, (val, key) => {
        if (!_.isNil(val)) {
            return fn(val, key);
        }
        return true;
